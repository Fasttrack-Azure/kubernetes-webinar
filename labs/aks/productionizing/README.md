...
📋 Start by deploying the app from `labs/aks/productionizing/specs/whoami`.


```
kubectl apply -f labs/aks/productionizing/specs/whoami
```
...
📋 Deploy the update in `labs/aks/productionizing/specs/whoami/update` and wait for the Pods with label `update=readiness` to be ready.


```
kubectl apply -f labs/aks/productionizing/specs/whoami/update
...
📋 Deploy the update in `labs/aks/productionizing/specs/whoami/update2` and wait for the Pods with label `update=liveness` to be ready.


```
kubectl apply -f labs/aks/productionizing/specs/whoami/update2
...
kubectl apply -f labs/aks/productionizing/specs/metrics-server
...
📋 Deploy the app from `labs/aks/productionizing/specs/pi`, check the metrics for the Pod and print the details for the HPA.


```
kubectl apply -f labs/aks/productionizing/specs/pi
...
```
kubectl apply -f labs/aks/productionizing/specs/configurable
```
...
```
kubectl apply -f labs/aks/productionizing/specs/pi-secure/
...
```
kubectl apply -f labs/aks/productionizing/specs/pi-secure/update
